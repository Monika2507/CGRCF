function featuremap=extract_features(patch,use_sz,hogfeatures,hogparams,frame,ifcompress,pe,params)

  x_vgg16= get_vggfeatures(patch,use_sz,23,params);
   x_vggm= get_vggmfeatures(patch,use_sz,4,params);
   
%    x_vgg16= get_features_resnet50(patch,use_sz,'res4fx',params);%get_vggfeatures(patch,use_sz,23,params);
%    x_vggm= get_features_resnet50(patch,use_sz,'res3dx',params);%get_vggmfeatures(patch,use_sz,4,params);
...
function feat=featPCA(feat,num_channels,mode)
    ...
    switch mode
        case 'vgg16'
            feat_=gather(feat_)*gather(vgg16coeff);
        case 'vggm'
            feat_=gather(feat_)*gather(vggmcoeff);
        case 'hc'
            feat_=gather(feat_)*gather(hccoeff);
    end
    feat=reshape(feat_,hf,wf,num_channels);
end
